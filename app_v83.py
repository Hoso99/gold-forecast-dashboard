...
from gold_model_v83 import MODEL_VERSION_V83, decide_v83
from macro_econometrics_v825 import combine_macro_sources, download_fred_macro, parse_slow_factor_csv

st.set_page_config(page_title="Gold V8.3 Research", page_icon="🟡", layout="wide")
st.title("Gold Version 8.3 — Two-Horizon Research Challenger")
st.caption("Separate research model · macro regime + one-hour timing · no broker execution")
...
try:
    with st.spinner("Running two-horizon walk-forward research…"):
        gold, confirmations, source_status = download_intraday_bundle(key)
        validate_market_data(gold, confirmations)
        result = fit_intraday_system(gold, confirmations, splits, cost_bps, threshold)
...
st.subheader("Two-horizon decision")
